...
void setup() {
  ...
  Person personOne;
  Person personTwo;
  // Bytes = 36 (firstName + lastNmae + id = 16 + 16 + 4)
  ...
  // We are working with a 256Kb byte EEPROM. That is we have 32,768 bytes to work with
  ...
  // ESTOP & FUSE are a special case. Their logic is inverted.
  ...
  attachInterrupt(digitalPinToInterrupt(ESTOP), interruptEstopLow, LOW);
  attachInterrupt(digitalPinToInterrupt(FUSE), interruptFuseLow, LOW);
...
void interruptEstopLow(){
  Serial.println("LOW Interrupt ESTOP!!");
  // Stop everything!!
