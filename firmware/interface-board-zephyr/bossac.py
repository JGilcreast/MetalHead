...
if platform.system() == 'Darwin':
    ...
    if not ports:
        print(f"No COM ports found. DEFAULT_BOSSAC_PORT={DEFAULT_BOSSAC_PORT}")
    ...
    for port in ports:
        if port.vid is not None and port.pid is not None:
            if port.vid == 9025 and port.pid == 61:
                ...
                print(f"  Vendor ID (VID): 0x{port.vid:04X}")
                print(f"  Product ID (PID): 0x{port.pid:04X}")
                DEFAULT_BOSSAC_PORT = port.device
                import serial
                try:
                        # Open the port at 1200 baud with DTR enabled
                        ser = serial.Serial(port.device, 1200, dsrdtr=True)
                        time.sleep(0.2)  # Give the device time to respond
                        ser.dtr = False  # Toggle DTR to Low
                        ser.rts = False  # Toggle RTS to Low
                        time.sleep(0.2)
                        ser.dtr = True   # Toggle DTR to High
                        ser.rts = True   # Toggle RTS to High
                        time.sleep(0.2)
                        ser.close()
                        print(f"1200-bps touch reset for {port.device} initiated.")
                except serial.SerialException as e:
                        print(f"Error opening or communicating with {port.device}: {e}")
...
class BossacBinaryRunner(ZephyrBinaryRunner):
    ...
    def __init__(self, cfg, bossac='bossac', port=DEFAULT_BOSSAC_PORT,
                 speed=DEFAULT_BOSSAC_SPEED, boot_delay=0, erase=True):
        super().__init__(cfg)
        self.bossac = bossac
        self.port = port
        self.speed = speed
